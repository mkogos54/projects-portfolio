...
bool BME280::ReadTrim()
{
   ...
   //success &= readBytes(TEMP_DIG_ADDR, TEMP_DIG_LENGTH, &m_dig[ord]);
   ord += TEMP_DIG_LENGTH;

   // Pressure Dig
   success &= ReadRegister(PRESS_DIG_ADDR, &m_dig[ord], PRESS_DIG_LENGTH);
   //success &= readBytes(PRESS_DIG_ADDR, PRESS_DIG_LENGTH, &m_dig[ord]);
   ord += PRESS_DIG_LENGTH;

   // Humidity Dig 1
   success &= ReadRegister(HUM_DIG_ADDR1, &m_dig[ord], HUM_DIG_ADDR1_LENGTH);
   //success &= readBytes(HUM_DIG_ADDR1, PRESS_DIHUM_DIG_ADDR1_LENGTHG_LENGTH, &m_dig[ord]);
   ord += HUM_DIG_ADDR1_LENGTH;

   // Humidity Dig 2
   success &= ReadRegister(HUM_DIG_ADDR2, &m_dig[ord], HUM_DIG_ADDR2_LENGTH);
   //success &= readBytes(HUM_DIG_ADDR2, HUM_DIG_ADDR2_LENGTH, &m_dig[ord]);
...
)
{
   ...
   //success = readBytes(PRESS_ADDR, SENSOR_DATA_LENGTH, buffer);
